    ...
    void Process(){
        ...
        while (true) {
            ...
            for (auto const& detection : detections){
                ...
                cv::Rect object(d[3], d[4], d[5]-d[3], d[6]-d[4]);
                printf("rect:[%f, %f, %f, %f]\n", d[3], d[4], d[5]-d[3], d[6]-d[4]);
...
public:
    explicit YoloExample(const cv::CommandLineParser &parser) : SimplePipeline(parser){
        ...
        settings.m_lostTrackType = tracking::TrackNone;       // Use KCF tracker for collisions resolving
    ...
    void DrawData(cv::Mat frame, double fontScale){
        for (const auto& track : m_tracker->tracks)
        {
                    ...
                    )
            {
                ...
                std::string label = track->m_lastRegion.m_type + ": " + std::to_string((int)(track->m_lastRegion.m_confidence * 100)) + " %" + "track_id:" + std::to_string(track->m_trackID);
